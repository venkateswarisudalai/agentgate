#!/usr/bin/env bash
# Install the agentgate Security Agent persona into Claude Code.
#
# What this does:
#   1. Confirms agentgate is built
#   2. Picks a settings.json target (~/.claude/settings.json by default)
#   3. Generates a settings.json with the PreToolUse hook + filesystem MCP
#      already wired through agentgate, with absolute paths filled in
#   4. Tells you what to do next
#
# Safety:
#   - If a settings file already exists at the target path, this script does
#     NOT modify it — it prints the generated config to stdout and you merge
#     manually. The only filesystem write happens when no settings file exists.

set -euo pipefail

# Resolve the agentgate repo root (this script lives in examples/security-agent/).
...
bold "agentgate Security Agent installer"
...
echo "  4. Start a fresh Claude Code session — the Security Agent hook is now active."
echo
gray "Tip: paste the contents of system-prompt.md as your first message in new"
gray "Claude Code sessions to give the agent the AppSec persona, until Claude Code"
